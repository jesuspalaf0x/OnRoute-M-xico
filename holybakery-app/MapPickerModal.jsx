...
const MAP_SUGGESTIONS = RAW_SUGGESTIONS.map(s => ({
  ...s,
...
export default function MapPickerModal({ onClose, onConfirm }) {
  ...
  const [pin, setPin] = useState({ 
    lat: MAP_SUGGESTIONS[0].lat, 
    lng: MAP_SUGGESTIONS[0].lng, 
    place: MAP_SUGGESTIONS[0] 
  });
  ...
  const filtered = MAP_SUGGESTIONS.filter((p) =>
  ...
  const updatePinFromCoords = (lat, lng) => {
    let best = MAP_SUGGESTIONS[0];
    let bestD = Infinity;
    MAP_SUGGESTIONS.forEach((p) => {
      const d = (p.lat - lat) ** 2 + (p.lng - lng) ** 2;
      if (d < bestD) { bestD = d; best = p; }
    });

    const isNear = bestD < 0.001;
    setPin({
      lat, lng,
      place: isNear ? best : {
        name: "Punto seleccionado en mapa",
        addr: "Coordenadas capturadas manualmente",
        zone: best.zone, custom: true
      }
    });
  };
  ...
  const zoneClass =
    pin.place.zone === "Especial" ? "status-pagada" :
    pin.place.zone === "Sin zona" ? "status-pendiente" :
    "status-confirmada";
          ...
          <aside className="mp-side">
            ...
            <div className="mp-list">
              ...
              {filtered.map((p) => {
                ...
                const pillCls = p.zone === "Especial" ? "status-pagada"
                              : p.zone === "Sin zona" ? "status-pendiente"
                              : "status-confirmada";
                return (
                  <button key={p.name} className={"mp-item " + (isActive ? "active" : "")} onClick={() => pickSuggestion(p)}>
                    <div className="mp-item-icon">
                      {p.pref ? (I.Star && <I.Star size={14}/>) : p.warn ? (I.AlertTriangle && <I.AlertTriangle size={14}/>) : (I.Pin && <I.Pin size={14}/>)}
                    </div>
                    <div className="mp-item-body">
                      <strong>{p.name}</strong>
                      <span>{p.addr}</span>
                    </div>
                    <span className={"status " + pillCls}>{p.zone}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="mp-foot-info">
            <div className="mp-foot-pin">
              ...
              <div>
                ...
                <span className="muted">{pin.place.addr}</span>
              </div>
            </div>
            <div className="mp-foot-meta">
              <span className={"status " + zoneClass}>{pin.place.zone}</span>
              <span className="mono mp-coords">{currentLat}, {currentLng}</span>
            </div>
          </div>
          <div className="mp-foot-actions">
            <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
            <button className="btn btn-primary" onClick={() => onConfirm(`${pin.place.name}, ${pin.place.addr}`)}>
